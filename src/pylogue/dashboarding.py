...
def render_plotly_chart_dataframes(plotly_python: str, **dataframes):
    ...
    try:
        ...
        script_html = (
            ...
            "const ORIGINAL_DATA = JSON.parse(JSON.stringify(fig.data));"
            "const ORIGINAL_LAYOUT = JSON.parse(JSON.stringify(fig.layout));"
...
snippets that start with `pd.DataFrame(data)`.
    ...
    try:
        ...
        script_html = (
            ...
            "try{"
            "Plotly.react(gd, JSON.parse(JSON.stringify(ORIGINAL_DATA)), JSON.parse(JSON.stringify(ORIGINAL_LAYOUT)));"
            "}catch(e){"
